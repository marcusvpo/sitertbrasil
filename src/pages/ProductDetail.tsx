...
const ProductDetail = () => {
  ...
  return (
    <section className="bg-secondary text-secondary-foreground py-8 md:py-16 min-h-[60vh]">
      <div className="container">
        <Button
          asChild
          variant="ghost"
          className="text-secondary-foreground/60 hover:text-secondary-foreground mb-6"
        >
          <Link to="/motorex">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12">
          ...
          <div>
            {/* Main image */}
            <div className="relative aspect-square rounded-lg overflow-hidden bg-secondary-foreground/5 mb-3">
              {images.length > 0 ? (
                <>
                  <img
                  ...
                  {images.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-2 top-1/2 -translate-y-1/2 bg-secondary/60 backdrop-blur-sm rounded-full p-2 text-secondary-foreground hover:bg-secondary/80 transition-colors"
                      >
                        <ChevronLeft size={20} />
                      </button>
                      <button
                        onClick={() => setActiveImage((p) => (p < images.length - 1 ? p + 1 : 0))}
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-secondary/60 backdrop-blur-sm rounded-full p-2 text-secondary-foreground hover:bg-secondary/80 transition-colors"
                      >
          ...
          <div>
            ...
            <h1 className="font-heading text-3xl md:text-4xl uppercase font-bold mt-1 mb-3">
              {product.name}
            </h1>

            <div className="flex items-center gap-3 mb-6">
              {product.badge && (
                <Badge className="bg-primary/10 text-primary border-0 font-heading uppercase tracking-wider text-xs">
                  {product.badge}
                </Badge>
              )}
              {product.volume && (
